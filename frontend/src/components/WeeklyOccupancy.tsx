...
export default function WeeklyOccupancy() {
  ...
  return (
    <div className="bg-white rounded-xl border border-slate-200 shadow-sm overflow-hidden">
      <div className="p-5 border-b border-slate-100 flex items-center justify-between bg-slate-50/50">
        <h2 className="text-lg font-semibold text-slate-800">Занятость залов на неделю</h2>
      ...
      <div className="overflow-x-auto">
        <div className="min-w-[1000px] p-5">
          <div className="grid grid-cols-[120px_repeat(7,1fr)] gap-4">
            ...
            {data.halls.map(hall => (
              <>
                <div key={hall.id} className="flex items-center font-medium text-slate-800 text-sm">
                  {hall.name}
                </div>
                {weekDays.map(day => {
                  const dayStr = format(day, 'yyyy-MM-dd')
                  const daySlots = data.slots.filter(s => s.date === dayStr && s.hall_id === hall.id)
